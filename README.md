...
사용자가 다양한 주제의 후기를 검색하고 공유할 수 있는 후기 커뮤니티 서비스입니다.
검색 유입, 탐색 경험, 성능 개선, 운영 안정성을 중심으로 프론트엔드 주요 기능을 구현했습니다.

- 서비스: [https://modu-review.com](https://modu-review.com)
- 담당 역할: 메인, 검색, 작성, 상세, 알림 등 주요 화면 및 핵심 기능 구현
- 개발 인원: 프론트엔드 2명, 백엔드 2명

## 핵심 성과

- SSR 병목을 ISR + CSR 하이브리드 렌더링으로 개선: p99 9.4초 → 45ms, 실패율 50% → 0%
- 사용자 시나리오 기반 테스트 532개 작성 및 CI 커버리지 검증 자동화
- 검색 결과 0건 문제를 AI 외부 리뷰 검색 챗봇으로 보완
- Sentry와 Slack 알림을 연동해 운영 환경 에러 감지 자동화
- Nginx 기반 Blue-Green 배포로 배포 다운타임 약 4~5초 → 1~2초 개선
...
| 모니터링        | Sentry                                          |
| 배포/운영       | AWS EC2, Nginx, PM2, GitHub Actions, CodeDeploy |

## 주요 기술 선택 이유

| 기술                    | 선택 이유                                                                                                            |
| --------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Next.js               | 검색 유입이 중요한 게시글 본문과 메타데이터를 서버에서 제공하기 위해 선택했습니다. 이후 단일 vCPU 환경에서 SSR 병목이 발생해 메인 페이지는 ISR + CSR 하이브리드 렌더링으로 전환했습니다. |
| TanStack Query        | 검색, 댓글, 북마크, 마이페이지처럼 사용자별로 달라지는 서버 상태를 클라이언트에서 캐싱하고 무효화하기 위해 사용했습니다.                                             |
| Zustand               | 로그인, 알림, 에러처럼 여러 화면에서 공유되는 클라이언트 상태를 독립적으로 관리하기 위해 사용했습니다.                                                       |
| AWS EC2 / Nginx / PM2 | 비용 예측이 가능한 환경에서 Next.js 서버를 직접 운영하고, Nginx 기반 Blue-Green 배포로 배포 중단 시간을 줄이기 위해 사용했습니다.                            |
| Sentry                | 운영 환경에서 재현이 어려운 클라이언트 에러를 빠르게 파악하고 Slack 알림으로 대응하기 위해 도입했습니다.                                                    |
...
### 테스트 도입 - 사용자 시나리오 기반 통합 테스트 작성과 CI 자동화 - [이동하기](#테스트-도입---사용자-시나리오-기반-통합-테스트-작성과-ci-자동화)

실제 사용자 상호작용과 비즈니스 요구사항이 화면에서 어떻게 동작하는지 검증하는 통합 테스트를 중심으로 도입하고, CI 환경에서의 테스트 및 커버리지 검증 자동화.

- 총 532개 테스트 케이스 작성 및 코드 커버리지 약 97% 달성
...
- GitHub Actions를 통해 `main` 브랜치에서는 테스트 통과 여부와 최소 커버리지 기준 확인, Pull Request 단계에서는 커버리지 변화 리포트 자동 생성

---

### AI 챗봇 - 검색 결과 없음으로 인한 이탈을 줄이기 위해 외부 리뷰 검색 기능 구현 - [이동하기](#ai-챗봇---검색-결과-없음으로-인한-이탈을-줄이기-위해-외부-리뷰-검색-기능-구현)
...
클라이언트에서 서버 캐시를 직접 무효화할 수 없는 문제를 해결하기 위해 쓰기 요청을 Next.js 서버로 우회시키는 프록시 패턴 적용.

- 첫 요청(MISS): 172ms 대비, 캐시 HIT: 59ms로 약 3배(172ms => 59ms)의 읽기 성능 개선

---

### 에러 중앙화를 통한 일관된 에러 UX 제공 - [이동하기](#에러-중앙화를-통한-일관된-에러-ux-제공---이동하기)
...
- 77회 배포 중 8회 실패했지만 서비스 중단 0회 유지
...
| 메인페이지 구성 |
| -- |
| <img src="https://github.com/user-attachments/assets/672b1c24-be4f-4274-a91f-2a877bd0041a" width="780px" /> |
...
| ISR 적용 이전 부하테스트 |
| -- |
| <img src="https://github.com/user-attachments/assets/0491a085-1365-4c25-a968-5eda56d34bf8" width="780px" /> |
...
| ISR 적용 이후 부하테스트 |
| -- |
| <img src="https://github.com/user-attachments/assets/2e0f2b21-475d-4db3-9e96-d9f82effdce9" width="780px" /> |
...
## 테스트 도입 - 사용자 시나리오 기반 통합 테스트 작성과 CI 자동화

<div align="center">

| 커버리지 수치 |
| -- |
| <img src="https://github.com/user-attachments/assets/898235de-fdd3-4aac-8186-89665c80f720" width="780px" /> |

| PR 커버리지 리포트 예시 |
| -- |
| <img src="https://github.com/user-attachments/assets/46d38a36-0f59-4719-86d9-e78d100b6461" width="780px" /> |
...
### 구현 1 - 사용자가 경험하는 기능 단위로 통합 테스트 작성
...
### 구현 2 - 세부 UI 분기와 복잡한 순수 로직은 유닛 테스트로 분리
...
### 구현 3 - GitHub Actions 기반 테스트 및 PR 커버리지 리포트 생성 자동화
...
이때 `test:ci` 스크립트 실행 과정에서 테스트가 하나라도 실패하거나, 전체 커버리지가 Jest 설정에 정의한 임계값(약 94%) 아래로 떨어질 경우 CI 파이프라인이 즉시 중단되도록 설정해 품질 이하의 코드 배포를 차단했습니다.
    ...
    steps:
      ...
      - name: Jest coverage report
        ...
        with:
          ...
          annotations: all
          custom-title: '모두의 후기 테스트 커버리지'
```

이 워크플로우는 병합 대상 브랜치가 기존 `main` 브랜치 대비 전체 커버리지를 얼마나 변화시켰는지와 파일별 커버리지 변화를 분석해 PR 댓글에 자동으로 리포트를 생성합니다.
...
보다 자세한 테스트 도입 배경과 구현 상세는 [블로그](https://gojimin.com/posts/frontend-test)에 정리했습니다.

## AI 챗봇 - 검색 결과 없음으로 인한 이탈을 줄이기 위해 외부 리뷰 검색 기능 구현

### 검색 결과가 없을 때 대체 탐색 경험 제공

<div align="center">

| 검색 결과가 없을 때 대체 탐색 경험 제공 |
| -- |
| <img src="https://github.com/user-attachments/assets/d248797e-cb8b-4290-a5b2-52c4cbe69ebc" width="780px" /> |
...
### 구현 1 - 검색 요청을 처리하는 Route Handler 구현

프로젝트의 백엔드 리소스가 부족해 이 기능만을 위한 별도 API를 새로 설계하기 어려웠습니다. 그렇다고 클라이언트에서 외부 AI 서비스를 직접 호출하는 방식은 API 키 노출 위험이 있어 적절하지 않았습니다.

그래서 검색 기능에 필요한 서버 역할을 프론트 서버가 담당하도록 구성했습니다. 클라이언트는 `/api/search`로 요청만 보내고, 서버가 이후 검증과 검색 로직을 실행한 뒤 결과를 반환하도록 구현했습니다.
...
한정된 사용량을 의미 없이 소진하지 않기 위해, 검색 이전 단계에서 LLM 기반 검증을 수행해 `keyword`와 `category`만으로 실제 후기 탐색에 적절한 요청인지 먼저 판단하도록 구성했습니다.
...
또한 외부 검증 API 지연이 길어질 경우 전체 대기 시간이 과도하게 늘어나는 것을 막기 위해 `Promise.race`를 사용해 타임아웃을 적용했습니다.
...
마지막으로 부적절한 요청은 실제 검색 이전에 차단하고 애매한 고유명사는 과도하게 막지 않도록 프롬프트를 설계해 검증 정확도를 높였습니다.

### 구현 3 - 카테고리별 suffix를 활용한 외부 리뷰 검색 및 요약
...
예를 들어 음식 카테고리에서는 `"프레드 피자" 맛 평가 양 가성비 솔직 후기 메뉴 추천`처럼 검색어를 보완해, 단순 정보성 결과보다 실제 후기 문서가 더 우선적으로 수집되도록 설정했습니다.

### 구현 4 - 검색 결과 없음 화면을 챗봇 진입점으로 전환
...
### 구현 5 - 복잡한 입력 단계를 대화로 숨기는 단계형 챗봇 UX 구현
...
구현 배경, 검색 공급자 선정 이유와 서버 구현 상세 내용은 [블로그](https://gojimin.com/posts/chatbot-1)에 정리했습니다.

챗봇 UX를 선택한 이유와 구현 상세 내용은 [블로그](https://gojimin.com/posts/chatbot-2)에 정리했습니다.
...
| 요청 구성도 |
| -- |
| <img src="https://github.com/user-attachments/assets/5d36974f-db94-4edc-baf9-153e00603ca9" width="780px" /> |
...
| 입력 시 타이핑마다 전체 컴포넌트 리렌더링 |
| -- |
| <img src="https://github.com/user-attachments/assets/85582e4e-38d2-4ab2-a0b3-ec3ef920c738" width="780px" /> |
...
| 최적화 이후 |
| -- |
| <img src="https://github.com/user-attachments/assets/2a2de7f7-9fb2-488c-acfe-f15f02aea7da" width="780px" /> |
...
#### 구현 1 - NodeView 기반 커스텀 이미지 업로드 노드 구현
...
#### 해결 방안 2 - presigned URL 기반 업로드 핸들러 주입
...
#### 구현 3 - 업로드 취소(AbortController) 지원원
...
| -- |
| <img src="https://github.com/user-attachments/assets/56e06735-d8ac-4317-9a77-52ca00cccbc7" width="780px" /> |

| 업로드 진행률 표시 및 취소 지원 |
| -- |
| <img src="https://github.com/user-attachments/assets/f908bca9-6160-43c0-8269-ad26a626a991" width="780px" /> |

| 예외 처리 통합 및 에러 UI 표시 |
| -- |
| <img src="https://github.com/user-attachments/assets/bde542d4-3e3e-43a5-b127-ad27f1aa7ef7" width="780px" /> |
...
| 에디터 재사용 |
| -- |
| <img src="https://github.com/user-attachments/assets/ada42ccf-902b-46e1-9e91-a2dcb9764ca7" width="780px" /> |
...
| 서식 지원 |
| -- |
| <img src="https://github.com/user-attachments/assets/9929a0ea-ce2b-46b7-aa1a-809ea65fc4bc" width="780px" /> |
...
| 실시간 알림 및 게시글 이동 |
| -- |
| <img src="https://github.com/user-attachments/assets/5ffc78ee-6bee-4609-9c27-63014e35526a" width="780px" /> |

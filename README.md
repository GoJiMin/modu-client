...
백엔드 API 병목의 가능성도 고려했으나 next.js의 fetch 캐싱으로 베스트 후기 데이터를 캐싱한 상태에서도 p99 응답 속도가 9.4s로 동일하게 측정되어 SSR 렌더링 과정 자체가 병목의 주요 원인임을 확인했습니다.
...
적용 후 동일한 조건의 부하테스트 결과 p99 응답 속도 9.4s => 47ms (약 200배 개선), 요청 실패율 50% => 0%로 1200개의 요청이 모두 성공했습니다.

...
- 내부 데이터가 부족해 검색 결과가 없는 상황에서도 외부 리뷰를 대신 찾아주는 대체 탐색 경험을 제공할 수 있게 됐습니다.
- 검색어 검증, 카테고리 선택, 리뷰 검색 및 요약 과정 등을 단계형 챗봇 UX로 풀어내, 복잡한 AI 검색 절차를 사용자가 직접 다루지 않아도 되도록 구현했습니다.
- 백엔드 리소스가 부족한 상황에서도 프론트 주도로 3일 만에 프로토타입을 구현하고 실제 서비스에 배포해, 핵심 기능의 가능성을 빠르게 검증했습니다.
